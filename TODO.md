...
- Stellarium `MercStarMap` / `fov-frame` / `eyepiece-fov`
- 相机焦长、像素尺寸、ROI；目镜焦距与视场角

**已完成（首版）**

- 「跟随赤道仪」可选：默认关闭，拖动画布暂停跟随；需要时再开跟随或「居中到赤道仪」。屏幕中心十字丝已去掉。
- 相机矩形视场框（黄虚线）、目镜圆形视场框（绿实线）；按当前缩放角径比例钉在屏幕中央。
- **Stellarium Plus 风格视场模拟**：底部面板选择望远镜 + 目镜/传感器预设（含已连接相机），搭配变化时星图框/圈实时更新；支持自定义焦距与传感器参数。
- 设置可持久化；默认开启视场叠加、**默认不跟随**。交互决策见 `docs/DESIGN_NOTES.md`。

**仍待**

- 目镜预设库。
- 不跟随模式下按真实投影把框钉在天空坐标上（可拖视图后框仍留在指向处）。
