...
- Stellarium `MercStarMap` / `fov-frame` / `eyepiece-fov` / `mount-reticle`
- 相机焦长、像素尺寸、ROI；目镜焦距与视场角

**已完成（首版）**

- 「跟随赤道仪」：星图锁定到赤道仪指向，屏幕中心十字丝指示当前指向。
- 相机矩形视场框、目镜圆形视场框；按当前缩放角径比例绘制。
- **由像元尺寸 + ROI + 焦距自动计算相机 FOV**（`OpticsFov`，与板解同一公式）；缺焦距时在视场设置里填写，与 `plate_focal_length_mm` 共用。
- 设置可持久化；默认开启相机视场框、跟随、以及「视场跟随相机参数」。
