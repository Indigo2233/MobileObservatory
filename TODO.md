...
- 选中天体后可启动精定：GOTO → 等帧拍照 → ASTAP 解析 → 星图居中到解算坐标 → 误差 >2′ 则 sync（或 SkyWatcher 纠偏 GOTO）→ 再对准，最多 5 轮。
- 收敛判据为解算天空位置相对目标的球面角距 ≤ 2′；可用全局 STOP 中止。
- 目视「同步」与单次 GOTO 仍保留；「视场居中 / 居中到赤道仪」收到展开详情里，默认只显示目标名 + 精定 + GOTO。

**验收标准**

- [x] 单元测试覆盖角距与纠偏坐标计算。
- [ ] OnStep / iOptron 真机：精定收敛到 ≤2′。
