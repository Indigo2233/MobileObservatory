...
- GOTO / 回零 / 定距 RA 期间在所有应用页面持续显示醒目的全局 STOP 按钮，并显示 STOPPING 状态。手动方向键不弹全局 STOP，避免与方向盘重叠导致断触。
...
- [x] STOP 在 GOTO / 回零 / 定距移动期间于各页面持续可见；手动方向键不弹全局 STOP。
...
- [x] 真机确认不同页面及 WebView 上方的全局 STOP 按钮触控正常。
...
- 选中天体后可启动「指向并居中」：GOTO → 等帧拍照 → ASTAP 解析 → 星图居中到解算坐标 → 误差大于停止阈值则 sync（或 SkyWatcher 纠偏 GOTO）→ 再对准，最多 5 轮。
- 目标、解算、sync、星图居中统一到 JNOW；ASTAP J2000 先岁差再比较。
- 默认停止误差 **6′**，星图观测工具可改（`star_map_precision_tolerance_arcmin`，1′–30′）。成功当轮若支持 sync 仍会写入解算位置。
- 目视「同步」与单次 GOTO 仍保留；「视场居中 / 居中到赤道仪」收到展开详情里，默认只显示目标名 + 指向并居中 + GOTO。

**验收标准**

- [x] 单元测试覆盖角距、纠偏、JNOW/J2000 混用与 6′ 默认容差。
- [ ] OnStep / iOptron 真机：指向并居中收敛到设定误差。
...
- `StarMapScreen` 视野设置入口
- Stellarium `MercStarMap` / `fov-current` / `fov-target`
- 相机焦长、像素尺寸、ROI；目镜焦距与视场角

**已完成（首版 + M1a 双光路）**

- 「跟随赤道仪」可选：默认关闭，拖动画布暂停跟随；需要时再开跟随或「居中到赤道仪」。屏幕中心十字丝已去掉。
- 主镜 / 导星两条光路独立配置；黄虚线目标框一直钉在屏幕中央（构图），绿实线当前框钉在赤道仪指向。拖开星图后绿框留在指向处。
- 观测工具面板切换光路；视场设置顶栏同样可选要编辑的光路，改副光路不必切显示。
- 已连接相机可挂到任一条光路。旧 `star_map_*` 键迁到主镜；默认显示导星。
- 板解焦距只在导星且为相机时，从视场设置写入。决策见 `docs/DESIGN_NOTES.md` 与 [`docs/STAR_MAP_DUAL_FOV_PLAN.md`](docs/STAR_MAP_DUAL_FOV_PLAN.md)。
...
- [x] 可跟随赤道仪指向；默认不跟随，拖动即暂停。
- [x] 目镜模式：输入视场角后显示圆形范围。
- [x] 相机模式：按宽高角径显示矩形范围。
- [x] 可从已连接相机 + 焦距自动估算 FOV（缺焦距时手填）。
- [x] 主镜 / 导星独立配置，切换后只显示一条光路。
- [x] 不跟随时当前框钉在赤道仪天空坐标上；黄虚线目标框一直显示在屏幕中央，不需要点选天体。

**风险/成本：** 低；自动估算依赖厂商上报的 `pixelSizeUm` 与用户填写的焦距。
