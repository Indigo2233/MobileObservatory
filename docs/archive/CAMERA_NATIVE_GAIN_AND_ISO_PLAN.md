...
> 状态：M1–M4 已落地，含模式/像素格式切换后重读增益、写入忙碌态、非法输入提示、旧 dB 键丢弃测试。工业 overlay 在单位已是 dB 时不再重复显示等效值。**M5 单反/无反（PTP）接入暂不实施。**
>
> 触发来源：现有相机接口将各厂商增益统一换算为 dB，界面数值与厂商文档、推荐增益和相机原生设置存在偏差。离散 ISO 控件模型保留在通用增益能力里，但不接入任何单反/无反相机。
...
- **单反/无反相机（Canon/Nikon/Sony PTP 等）接入。** 通用增益控件仍支持离散 ISO 档位，但不枚举、不连接、不拍摄这类机身。
...
[-] **暂不实施（增益仓库范围内）。** 机身接入见独立方案 `docs/DSLR_MIRRORLESS_CAMERA_PLAN.md` 与 `TODO.md` 事项 18。通用 `GainControlKind.ISO` 与离散滑条保留在能力模型中，供该方案复用。
...
- [x] 为各适配器添加映射测试；换算与自动曝光边界已覆盖，真机 SDK 验证仍待插机。
...
- [x] 更新日志中的单位与格式。
- [x] 增加旧键废弃、自动曝光和 FITS 头测试；通用规范化测试已完成，设备与文件回归待真机阶段。
...
交付结果：本期不接入单反/无反。通用离散 ISO 控件仍留在增益模型中，不作为本里程碑验收项。机身接入见 `docs/DSLR_MIRRORLESS_CAMERA_PLAN.md`。
...
11. 相关 JVM 测试、Compose UI 测试和硬件冒烟测试通过。
...
5. [-] 单反/无反接入：不在本增益计划内，见 `docs/DSLR_MIRRORLESS_CAMERA_PLAN.md`。
