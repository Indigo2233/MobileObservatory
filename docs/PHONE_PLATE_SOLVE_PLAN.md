...
面向约 24 mm 等效主摄，采用以下顺序。每一步都保留可见诊断结果，避免将拍摄失败、星点不足和图案匹配失败混为同一种“解算失败”。
...
| P3 | 24 mm 专用本地宽场索引、IMU 先验三角匹配、候选验证 | [ ] | 离线解算输出 RA/Dec/旋转/FOV 和残差，错误候选必须拒绝 |
...
**测试数据集**：建立 `docs/phone-solve-dataset/` 的清单文件和受控存储的原图集。每个样本记录机型、镜头 ID、等效焦距、输出尺寸、实际曝光/ISO、RAW/YUV、UTC 中点、经纬度、手机姿态，以及由 W08 或 astrometry.net 取得的参考解。首批目标为 30 组 24 mm 主摄样本，覆盖城市、郊区、无月和有月。
...
- **基线镜头是约 24 mm 等效主摄**：按常见 4:3/3:2 输出，目标覆盖约 45°–85°视场范围；100 mm 等效长焦（约 20°×14°）属于可选增强，不能作为兼容性基线。
- **曝光受限基线是单帧 0.5 s**：连续采集 8–16 帧，完成暗场/热像素抑制、陀螺辅助配准与叠加，再做星点提取。单帧解算仅作为亮星充足时的快速路径。
- **宽场解算优先级**：24 mm 主摄使用面向约 45°–85°生成的 tetra3 亮星索引；ASTAP W08 作为交叉验证与过渡实现。G05 面向约 6°–20°，D50 保留给更窄的主相机/导星相机路径。

- 宽场星表：ASTAP W08 面向大于约 20°的视场，可用于 24 mm 等效主摄的过渡验证；正式手机路径使用为约 45°–85°视场生成的 tetra3 索引。
