...
> 状态：**实施中**。截至 2026-08-12，四步推镜界面、手动相机选择、单帧全手动拍摄、FITS/DNG 诊断输出、星点提取与星点叠加已经完成；宽场本地解算器仍在实施。
>
...
面向手机内置后置镜头的动态宽场采用以下顺序。24 mm、35 mm 等效镜头都按实际 Camera2 焦距、传感器有效区域和输出裁切计算视场。详细算法与路由见 `docs/PHONE_WIDE_FIELD_SOLVER_IMPLEMENTATION.md`。每一步都保留可见诊断结果，避免将拍摄失败、星点不足和图案匹配失败混为同一种“解算失败”。

| 阶段 | 交付物 | 当前状态 | 验收方式 |
|---|---|---|---|
| P0 | 四步推镜流程、镜头选择、曝光/ISO/RAW、拍摄图和星点覆盖层 | [x] | Debug APK 手工冒烟；单元测试与构建通过 |
| P1 | 8–16 张短曝光连续采集、保留采集元数据、堆叠前后诊断 | [~] | 0.5 s 上限机型可获得完整帧组，单帧与堆叠星点数可比较 |
| P2 | 热像素/背景处理、可重复的星点检测和真实图像回归集 | [~] | 每张测试图输出星点数、背景噪声与覆盖图；失败可复现 |
| P3 | 按动态视场分桶的本地宽场索引、无 IMU 盲解、可选 IMU 先验 | [ ] | 离线解算输出 RA/Dec/旋转/FOV 和残差，错误候选必须拒绝 |
| P4 | 以解算结果校正手机姿态，推镜会话在停手后重解 | [ ] | 电控赤道仪真值台和 Dob 实机联合验收 |

**P1 的范围约束**：首版在一次打开的 Camera2 session 内连续拍摄，以免每张图都承担打开相机的延迟和功耗。首版只在手机与镜筒静止时进行像素堆叠；陀螺辅助配准在 P4 的姿态时间线完成后接入。帧间运动超过阈值时，产品应丢弃该帧或退回单帧，不能把拖线星叠入结果。

**测试数据集**：建立 `docs/phone-solve-dataset/` 的清单文件和受控存储的原图集。每个样本记录机型、镜头 ID、物理焦距、等效焦距、有效裁切、输出尺寸、实际曝光/ISO、RAW/YUV、UTC 中点、经纬度、可选手机姿态，以及由 W08 或 astrometry.net 取得的参考解。首批目标为至少 30 组样本，覆盖可获得的约 14/24/35/50 mm 镜头、城市、郊区、无月和有月。

**阶段性目标**：P2 后，在检测到至少 8–10 个可信星点的实拍帧上完成稳定诊断；P3 后，郊区样本的解算成功率目标为 80%，中心误差小于 0.5°。这些是开发目标，必须由实拍数据集复核后才可对用户宣称。
...
- **基线输入是手机内置宽场镜头**：覆盖常见约 14–70 mm 等效物理镜头，依据每次拍摄的真实镜头、传感器有效区域与裁切动态计算视场；24 mm 与 35 mm 都是首版必须覆盖的路径。
- **曝光受限基线是单帧 0.5 s**：连续采集 8–16 帧，完成暗场/热像素抑制、陀螺辅助配准与叠加，再做星点提取。单帧解算仅作为亮星充足时的快速路径。
- **宽场解算优先级**：手机使用按动态视场分桶的全天亮星几何索引，必须支持无 IMU 盲解；可信 IMU 只用于缩小候选和加速。ASTAP W08 作为交叉验证工具。外接焦距不低于 200 mm 的导星镜、望远镜与成像相机走 ASTAP D20/D50 路径。

- 宽场星表：ASTAP W08 可用于桌面交叉验证；正式手机路径使用覆盖约 25°–125°对角视场的重叠索引桶。
...
| `targetSdk` 已升至 34；Camera2 与运行时权限需在 Android 12–15 真机冒烟 | 与 `TODO.md` 事项 4 同步推进硬件验证 |
