...
[-] **暂不实施。** 不接入单反或无反机身。通用 `GainControlKind.ISO` 与离散滑条保留在能力模型中。日后立项见仓库 `TODO.md` 事项 18；本期不增加 PTP 栈、VID 枚举或 Live View。
...
交付结果：本期不接入单反/无反。通用离散 ISO 控件仍留在增益模型中，不作为本里程碑验收项。机身接入跟踪见 `TODO.md` 事项 18。
