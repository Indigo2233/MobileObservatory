...
> 状态：**M0–M4 代码已落地**。构建与单元测试通过；M0 真机冒烟与 M5 长跑待插 Player One 验收。ZWO / QHY 实现未改动，不做额外回归。
>
...
- [ ] 真机：临时/正式连接路径跑通单帧或预览（`PlayerOneSdkHost.runSingleFrameDiagnostic` 已封装，亦可直接走设备选择器连接）
...
- [ ] 真机：主+导星两台 Player One 验证 `claim/release`（代码路径已具备）

### M5 —— 稳定性与长跑 `[ ]`

- [ ] 采集中热拔插、反复插拔、权限拒绝后重试
- [ ] 长时间连续采集的内存与丢帧观察
- [ ] Android 版本矩阵回归（厂商已在 Android 10 与 Android 16 上跑通，我们需覆盖自有机型）
- [ ] 供电场景对比：手机直连 vs 独立供电 USB Hub

**验收**：2 小时连续采集无泄漏、无崩溃；采集中拔线不崩且能重连。
